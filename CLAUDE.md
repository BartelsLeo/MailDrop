...
- Suggestion-relevant features are split into two groups: fixed features (Betreff, Datum, AusfueDatum, AbsenderDomain, Absender, AusfueBenutzer) are computed once per mail selection via dedicated per-feature subs called from PrepareSession(); mutable features (Titel, AbsenderKurz, AblageordnerAufgeloest, MsgDateinameSchema, ProjektPfad, ProjektstrukturPfad) are recomputed via dedicated per-feature subs triggered from the corresponding Session property setters and are zero-initialized at engine construction.
- Distance list allocation and initial computation are owned by CalculateInitialFeatureDistances(session) on SuggestionEngine, called once from PrepareSession() immediately after New(). New() handles only engine infrastructure: loading historical SessionRecords from the database (which carry pre-persisted BetreffEmbedded values) and instantiating EmbeddingService. No embeddings are computed in New(); the only runtime embedding generated is the current mail's Betreff, computed lazily in GetOrCreateCurrentBetreffEmbedding() during CalculateInitialFeatureDistances().
- SuggestionEngine exposes SuggestProjektPfad, SuggestProjektstrukturPfad, SuggestTitel, SuggestAblageordnerSchema, SuggestMsgDateinameSchema, and SuggestAnhaengeAblegen, all backed by a shared FindBestRecord helper that accepts a per-call weight dictionary and scores all historical records to return the best match. FindBestRecord takes explicit weights; a _defaultWeights dictionary is used for all String-field suggestions; SuggestAnhaengeAblegen uses its own domain-weighted dictionary emphasizing AbsenderDomain/Absender/AusfueBenutzer. SuggestAnhaengeAblegen returns Boolean? (Nothing when no records exist).
- Session exposes SuggestXxx(value) methods that set the field value and then set the corresponding IsSuggestedXxx boolean flag to True. Cascade is driven by property setters: ProjektPfad → ProjektstrukturPfad → Titel → AbsenderKurz → AblageordnerSchema → MsgDateinameSchema. After SuggestProjektPfad completes in PrepareSession, SuggestAnhaengeAblegen is called directly (not part of the setter cascade). SuggestAblageordnerSchema also updates AblageordnerFeld to the resolved value; SuggestMsgDateinameSchema also updates MsgDateinameFeld. Session also exposes IsSuggestedProjektPfad, IsSuggestedProjektstrukturPfad, IsSuggestedTitel, IsSuggestedAbsenderKurz, IsSuggestedAblageordnerSchema, IsSuggestedMsgDateinameSchema, IsSuggestedAnhaengeAblegen as full properties with PropertyChanged notification.
...
- SuggestionEngine uses weighted scoring across Betreff (semantic cosine similarity), Datum (normalized date similarity), AusfueDatum (normalized filing-date similarity), AbsenderDomain (categorical match), Absender (categorical match), AusfueBenutzer (categorical match), Titel (text similarity), AbsenderKurz (categorical match), Ablageordner (text similarity), MsgDateinameSchema (text similarity), ProjektPfad (categorical match), and ProjektstrukturPfad (categorical match).
- Feature weights are passed per suggestion call. _defaultWeights in SuggestionEngine is used for all String-field suggestions; SuggestAnhaengeAblegen uses its own weights that emphasize AbsenderDomain (0.3), Absender (0.2), AusfueBenutzer (0.2). Both weight dictionaries include AusfueDatum at 0.05. Weights do not need to sum to 1.0 — only relative ranking matters.

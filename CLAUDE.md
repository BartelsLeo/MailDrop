...
- Session exposes SuggestXxx(value) methods that raise the SuggestionApplied event (carrying the field name) before applying the value, enabling the UI to animate the corresponding sparkle indicator. Cascade is driven by property setters: ProjektPfad → ProjektstrukturPfad → Titel → AblageordnerSchema.
- MailDropWpfTaskPane subscribes to Session.SuggestionApplied and animates a ✦ sparkle icon next to the affected field. The sparkle fades in on suggestion and fades out when the user focuses/interacts with the field.

...
- **Known failure mode: installing/updating from a OneDrive-synced folder.** The two supported install locations are the plain network drive and a locally extracted zip (see above) — installing from (or auto-syncing the network-drive copy into) a OneDrive-backed folder, e.g. `...\OneDrive - <Org>\...\MailDrop\`, is not a supported third option and has caused both symptoms below, since it is the same underlying ClickOnce download step in both cases: on first run, `ClickOnceAddInDeploymentManager.InstallAddIn` performs the one-time copy of files from the install location into the local VSTO add-in cache; on every subsequent Outlook start, the background update check (see auto-update above) re-runs effectively the same download against that install location as the update source.
  - Symptom A: "update not possible" errors when a newer version is published, even though a fresh install from the same location works.
  - Symptom B: right after a fresh install, Outlook auto-disables the add-in due to a load timeout (see `DisabledItems` below), together with `System.Deployment.Application.DeploymentDownloadException` wrapping `System.UnauthorizedAccessException: Der Zugriff auf den Pfad "...\AppData\Local\Temp\Deployment\<random>\<random>\MailDrop.dll" wurde verweigert`.
  - Root cause: `System.Deployment.Application`'s download manager stages every downloaded file under a random subfolder of `%LOCALAPPDATA%\Temp\Deployment\` before verifying/copying it into the ClickOnce cache; a sharing violation on that just-written staging file is a well-known ClickOnce failure most often caused by endpoint antivirus/EDR real-time scanning briefly locking newly written `.dll`/`.exe` files (the deployment stack does not retry on this), compounded when the *source* files being read live in a OneDrive-synced folder whose Files On-Demand placeholders may not be fully hydrated. This happens inside Outlook's own VSTO loader, before `ThisAddIn_Startup` runs, so none of this project's own Try/Catch guards (see below) can intercept or work around it — **it is not fixable via code in this repository.**
  - Remedy: install/update only from the plain network drive path or a locally extracted zip that is **not** inside any OneDrive-synced folder. If it already failed: add an antivirus exclusion for `%LOCALAPPDATA%\Temp\Deployment\`, clear the ClickOnce cache (`rundll32.exe dfshim.dll CleanOnlineAppCache`), remove the stale `DisabledItems` registry value for MailDrop (see below), then reinstall from a non-OneDrive-synced location.

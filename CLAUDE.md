...
- The info popup (help button `i`) is organized into five headed sections: "Ablageordner und msg Dateiname" (schema/placeholder usage, including the quoting syntax for fixed text (`"..."`) and how an unquoted separator between two placeholders is dropped when a neighboring placeholder is empty - see Placeholder reference below), "Vorschläge" (suggestions are generated for all fields and marked with ✦, based on prior mail filings), "Vorschlagsdaten exportieren" (SuggestionRecord transfer, copy `%APPDATA%/MailDrop/sessions.db`), "Vorschläge verbessern" (contains the "Gewichte neu berechnen" button with explanation; the button is disabled while recalculation runs and re-enabled on completion via Dispatcher.Invoke), and "Fehlermeldung" (a plain-text line with a `mailto:leonard.bartels@posteo.de` Hyperlink inline in a BodyText TextBlock; InfoPopup.xaml.vb's Hyperlink_RequestNavigate opens the link via `Process.Start(New ProcessStartInfo(uri) With {.UseShellExecute = True})` since WPF Hyperlinks don't navigate on their own, wrapped in Try/Catch with Logger.LogError so a missing default mail client can't crash the popup). Below the five sections, a small dezent gray (`#9CA3AF`, 10px, centered) `TxtVersion` TextBlock shows the running version, set in InfoPopup's constructor: `GetDisplayVersion()` returns `ApplicationDeployment.CurrentDeployment.CurrentVersion` (the ClickOnce `ApplicationVersion`, auto-incremented per publish) when `ApplicationDeployment.IsNetworkDeployed` is True, else falls back to the assembly's static `AssemblyVersion` (1.0.0.0, unversioned local/F5 debug runs) — this requires the `System.Deployment` framework reference added to MailDrop.vbproj.

...
- **`ThisAddIn_Startup` never touches `Application.Explorers`/`ActiveExplorer()`/`Session` synchronously.** Confirmed via Outlook's own resiliency diagnostic (the "Dieses Add-In hat den langsamen Start von Outlook verursacht" message, with a `Schwellenwertzeit` of 1000 ms and observed `Benötigte Zeit` of 1765-2032 ms across separate occurrences): the synchronous `_explorers = Application.Explorers` call used to be the first line of `Startup` and can block on Outlook's own mail-store/session init (Exchange cached-mode sync, PST/OST mounting), which is variable-duration and was pushing `Startup` well past Outlook's slow-add-in threshold, causing MailDrop to be auto-disabled. This is a separate, distinct root cause from the ClickOnce `Temp\Deployment` access-denied failure documented under Distribution above — that one prevents the DLL from loading at all (fails before `Startup` ever runs); this one is the add-in loading successfully but too slowly. Both happen to produce the same *symptom* (Outlook disables MailDrop with a timeout-flavored message) but need to be diagnosed independently; don't assume a recurrence is the same cause without checking Outlook's own disabled-item message for the specific reason text and numbers. Fix: the `Explorers`/`_currentExplorer` wiring is deferred to a short (100 ms) `System.Windows.Forms.Timer` tick, the same UI-thread-deferral pattern `PreloadTaskPaneInBackground` already used for task pane creation (a `Timer` tick is COM-safe for STA-affine `Explorer` objects; `Task.Run` is not, since it would hand the COM call to a non-UI thread). `MailAblegen_Click`'s ribbon-click path is unaffected since it doesn't depend on `_currentExplorer`; only automatic `SelectionChange` reactivity is delayed by the same ~100 ms at startup.

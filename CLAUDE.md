...
- Session.ReplacePlaceholders splits the schema template into alternating literal/placeholder segments (by matching the known placeholder tokens) rather than doing plain sequential String.Replace calls, and implements a "skip-empty-join": a literal segment that sits directly between two placeholders (a "connector", e.g. the "_" a user types between placeholders, as in the default schema) is held back (pendingConnector) instead of appended immediately, and only actually emitted right before the next non-empty placeholder that follows a previously-emitted non-empty placeholder. An empty placeholder is skipped without discarding a still-pending connector (a later connector simply overwrites it). This guarantees exactly one connector between any two placeholders that actually resolved to a value, however many empty placeholders sit between them, and never leaves a leading or trailing connector. Literal text that borders only one placeholder (e.g. a fixed prefix before the first placeholder) is not a connector and is always emitted unchanged, even if that placeholder is empty - the separator-collapsing behavior is intentionally scoped to text the user placed between two placeholders, since arbitrary literal text is not known to be a "separator" by the code. Example: "[A]_[B]_[C]" with only B empty resolves to "A_C" (not "AC" or "A__C"); with both B and C empty it resolves to "A" (no trailing "_").

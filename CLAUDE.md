...
- **Known failure mode: `Temp\Deployment` access-denied during install/update.** Observed symptoms: (A) "update not possible" errors when a newer version is published even though a fresh install from the same location works; (B) right after a fresh install, Outlook auto-disables the add-in due to a load timeout (see `DisabledItems` below), together with `System.Deployment.Application.DeploymentDownloadException` wrapping `System.UnauthorizedAccessException: Der Zugriff auf den Pfad "...\AppData\Local\Temp\Deployment\<random>\<random>\MailDrop.dll" wurde verweigert`. Both are the same underlying ClickOnce download step failing: on first run, `ClickOnceAddInDeploymentManager.InstallAddIn` copies files from the install location into the local VSTO add-in cache; on every later Outlook start, the background update check (see auto-update above) re-runs effectively the same download against that install location.

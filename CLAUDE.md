...
- Creating a folder (Neuer Ordner / Neuen Ordner) inserts the new node directly into the existing TreeViewData tree via DirectoryTreeHelper.InsertDirectoryNode / Session.InsertDirectoryTreeNode, instead of rebuilding the whole tree from disk with BuildDirectoryTree(). This avoids both the cost of a full filesystem re-scan and the loss of the user's manually expanded/collapsed node state, which a full rebuild causes because CreateDirectoryNodeWithExpand unconditionally resets IsExpanded for the first two levels on every call. The new node is inserted alphabetically (case-insensitive) among its siblings; the parent node is marked expanded so the new node becomes visible. Falls back to the old full BuildDirectoryTree() rebuild if the parent node can't be located in the current tree (defensive); if the target folder already existed on disk, insertion is skipped entirely since it must already be present in the tree. Delete (Loeschen) and rename (Umbenennen) still call the full BuildDirectoryTree() rebuild and are not yet optimized this way.
...
- Session.ReplacePlaceholders splits the schema template into alternating literal/placeholder segments (by matching the known placeholder tokens) rather than doing plain sequential String.Replace calls. When a placeholder resolves to an empty value, a literal segment that sits directly between that placeholder and a neighboring one (e.g. the "_" a user types between two placeholders, as in the default schema) is dropped along with it. This avoids a leftover separator at the start/end of the resolved string when the first/last placeholder is empty, and avoids a doubled separator when a placeholder in the middle is empty. Since the separator is arbitrary user-typed text (not a structural join character known to the code), this is a heuristic: it only removes a literal segment that has a placeholder on both sides; literal text adjacent to only one placeholder (e.g. a fixed prefix before the first placeholder) is left untouched even if that placeholder is empty. A placeholder empty in the middle of the schema removes both surrounding separators rather than merging into a single one (e.g. "[A]_[B]_[C]" with B empty resolves to "AC", not "A_C").
